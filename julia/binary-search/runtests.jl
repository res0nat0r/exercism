...
@testset "default binary search" begin
    @testset "value in array" begin
        ...
        @test binarysearch([1, 3, 5, 8, 13, 21, 34, 55, 89, 144, 233, 377, 634], 144) ==
              10:10
    ...
    @testset "bonus tasks" begin
        if get(tested_bonus_tasks, :rev, false)
            @testset "reverse search" begin
                @testset "value in array" begin
                    ...
                    @test binarysearch(
                        [634, 377, 233, 144, 89, 55, 34, 21, 13, 8, 5, 3, 1],
                        144,
                        rev = true,
                    ) == 4:4
                    @test binarysearch(
                        [377, 233, 144, 89, 55, 34, 21, 13, 8, 5, 3, 1],
                        21,
                        rev = true,
                    ) == 7:7
                end
        ...
        if get(tested_bonus_tasks, :multiple_matches, false)
            @testset "multiple matches" begin
                ...
                @test binarysearch(
                    [1, 3, 5, 8, 13, 21, 34, 55, 144, 144, 144, 377, 634],
                    144,
                ) == 9:11
                @test binarysearch([1, 3, 5, 21, 21, 21, 21, 21, 89, 144, 233, 377], 21) ==
                      4:8
            end
        end
    end
